...
Start by importing the complete gapminder dataset that we used yesterday into our fresh new R session: `gapminder_data <- read_csv('data/gapminder_data.csv')`.
...
We name this new column so we can use in a future argument. So the `averageLifeExp=` part tells `summarize()` to use "averageLifeExp" as the name of the new column. Note that you don't have to have quotes around this new name as long as it starts with a letter and doesn't include a space.
...
> > ```
> >
> > We see here that the first year in the dataset is 1952. Filter to only 1952, and determine the average GDP per capita.
> >
> > ```{r FilterChallenge2}
> > gapminder_data %>%
> > filter(year == 1952) %>%
...
> ## `mutate()`
> We can also multiply by constants or other numbers using mutate - remember how in the plotting lesson we made a plot with population in millions? Try making a new column for this dataframe called popInMillions that is the population in millions. 
> 
> > ## Solution: 
> > ```{r mutateExercise}  
> > gapminder_data %>%  
...
> >
> > Find a helper function on the help page that will choose all the columns that have "p" as their last letter (i.e.: "pop","lifeExp","gdpPerCap")
> >
> > > ## Solution
> > >
