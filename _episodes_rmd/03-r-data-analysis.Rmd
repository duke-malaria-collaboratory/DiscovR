...
Start by importing the complete gapminder dataset that we used yesterday into our fresh new R session: `smoking <- read_csv('data/smoking_cancer.csv')`.

What error do you get and why? Fix the code so you don't get an error and read in the gapminder dataset.
...
>   summarize(mean_smoke_pct=mean(smoke_pct))
> ```
> Or if we want to change `gapminder_data` itself (DON'T RUN THIS): 
...
>
> Use the piping function and `summarize()` to find the most recent year in the data set. HINT: the `max()` function returns the maximum value.
>
> > ## Solution:
> >
> > ```{r PipeExercise}
...
So we see that the most recent year in the dataset is `r smoking %>% summarize(recent_year=max(year)) %>% pull(recent_year)`. Let's calculate the average percent of smokers for all countries for only that year. To do that, we will use the `filter()` function to only use rows for that year before calculating the mean value.

```{r GapRecentLifeExp}
smoking %>%
  filter(year = 2019) %>%
  summarize(mean_smoke_pct_2019=mean(smoke_pct))
```
...
So here R thinks we're trying to assign 2019 to year, kind of like we do in the summarize function.
What we really want to do is find all of the years that are equal to `r smoking %>% summarize(recent_year=max(year)) %>% pull(recent_year)`
...
>
> What is the average percent of people with lung cancer for the first year in the dataset? *Hint: the column headers identified by `read_csv()` showed us there was a column called `lung_cancer_pct` in the dataset*
>
> Bonus: Figure out a way to write the code so you don't have to type the year into filter. (This is called hard-coding and we try to avoid it because if the data changes the number might also change.)
>
> > ## Solution
> >
...
> > ```
> >
> > Filter to only the minimum year, and determine the average percent of people with lung cancer.
> >
> > ```{r FilterChallenge2}
> > smoking %>%
> > filter(year == 1990) %>%
...
> > ```
> > {: .source}
> > By combining `filter()` and `summarize()` we were able to calculate the mean mercent of people with lung cancer in 1990.
> > 
> > Bonus:
...
smoking %>%
  ...
  pivot_wider(names_from = year, values_from = smoking_pct)
```
...
Let's make a plot with our wide data comparing life expectancy in 1952 to life expectancy in 2007 to see whether life expectancy has increased for all countries.
...
It seems like there are a couple countries where life expectancy has decreased since 1952. Let's figure out which those are!

> ## Bonus: Identifying countries with a lower life expectancy in 2007 than 1952
>
> Use what you've learned from today to figure out which countries had a lower life expectancy in 2007 than 1952. 
> 
> > ## Solution
> > ```{r}
> > gapminder_data %>%
> >   select(country, continent, year, lifeExp) %>% # select the columns you want to keep
> >   pivot_wider(names_from = year, values_from = lifeExp, names_prefix = 'y') %>% # pivot wider
> >   mutate(diff = y2007 - y1952) %>% # find the difference between the years of interest
> >   select(country, continent, diff) %>% # select the columns of interest
> >   filter(diff < 0) # filter to ones where the difference is less than zero
> > ```
> {: .solution}
{: .challenge}
...
> Read in the `gapminder_data.csv` file, filter to include only the year 2007 and the continent "Americas." Then drop the `year` and `continent` columns from the dataframe. Then save the new dataframe into a variable called `gapminder_data_2007`. 
> 
> > ## Solution: 
