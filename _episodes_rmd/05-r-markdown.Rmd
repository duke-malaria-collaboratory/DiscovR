...
> ## Association between air pollution and lung cancer
> Create a new code chunk and make a plot with air pollution per capita on the x axis and lung cancer on the y axis. 
> > ## Solution
...
smoking_pollution %>%
  filter(year == max(year)) %>% 
  ggplot(aes(x = air_pollution, y = lung_cancer_pct, color=continent, size=pop/1000000)) + 
  geom_point() +
  labs(x = "Air pollution per capita", y = "Percent of people with lung cancer",
       title= "Are lung cancer rates associated with air pollution?", size="Population (in millions)")
```
...
> 
> ```
> There are `r '\x60r n_countries \x60'` countries in our dataset. 
> ```
