...
now see a trend in the data. It looks like countries with greater smoking rates tend to have higher lung cancer rates. Let's add a title to our plot to make that
...
Here we can see that in 1990 the African countries tended to have much smoking and lung cancer rates than many other continents. Notice that when we add a mapping for
...
what effect population might have on smoking rates and lung cancer rates. Do you
think larger countries will have a greater or lower lung cancer rates? Let's find
...
There doesn't seem to be a very strong association with population size. We got another legend here for size,
...
> Now create your own line plot comparing population and lung cancer rates! Looking at your plot, can you guess which two countries have experienced massive change in population from 1952-2007?
>
> > ## Solution
> > ```{r smokingMoreLines}
> > ggplot(data = smoking) +
...
We have so many observations/data points, that even making the points transparent doesn't really help us see them! Another option is to "jitter" the points. This adds some random variation to the position of the points so you can see them better. We can do this using `geom_jitter()`.
...
Since we are plotting a discrete value on the x axis, and a continuous value on the y axis, we will need to tell `geom_jitter()` not to change the y value positions. We can do this by setting `height = 0` inside the geom:
...
base_plot +
  geom_boxplot() +
  geom_jitter(aes(color = continent), height = 0, alpha = 0.1)
```
...
> ```{r}
> ggplot(data = smoking, mapping = aes(x = continent, y = lung_cancer_pct)) +
>   geom_boxplot() +
>   geom_jitter(height = 0, alpha = 0.1)
> ```
{: .callout}
...
The other faceting function ggplot provides is `facet_grid()`. The main difference is that `facet_grid()` will make sure all of your smaller boxes share a common axis. In this example, we will put the boxes into columns side-by-side so that their y axes all line up.
...
3. Write down 3 questions you could answer with your data set.
4. How can you use visuals to explore those questions? Draw 3 plots out on paper. What will these plots show you?
5. Is your data in a format that can easily be read into R? How can you improve the formatting of the contents of the file? 
